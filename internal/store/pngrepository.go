...
// findPng - retrieves the PNG card for the given CardID and corresponding with the given timestamp
...
// findThumbnail - retrieves the PNG thumbnail for the given CardID
func (r *pngRepository) findThumbnail(cardID scheme.CardID) (image.Image, error) {
	// Retrieve the path of the thumbnail for the given CardID
	cardFilePath := r.getThumbnailPath(cardID.String())
	// Retrieve the thumbnail image
	return filex.ReadImage(cardFilePath)
}

// findAllPngsForID - returns a list with all the timestamps (sorted descending from newest to oldest) for the given CardID
...
func (r *pngRepository) findAllPngsForID(cardID scheme.CardID) []timestamp.Nano {
	...
	// Retrieve the root path of the given CardID
...
// deletePng - remove the PNG card of the given CardID, corresponding with the given timestamp
func (r *pngRepository) deletePng(cardID scheme.CardID, timestamp timestamp.Nano) error {
	// Retrieve the PNG card path of the given CardID, corresponding with the given timestamp
	cardFilePath := r.getCardPngPath(cardID, timestamp)
...
// deleteAllPngs - delete all PNG data of the given CardID (including thumbnail)
...
// for the given CardID is not above maxVersions
