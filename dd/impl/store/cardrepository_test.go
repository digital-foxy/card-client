...
func TestCardRepository_DataIntegrity(t *testing.T) {
	...
	assert.NotZero(t, retrievedCard.CheckTime, "SyncTime should be set")
	assert.Equal(t, scheme.UpdateSuccess, retrievedCard.LastUpdateStatus, "LastSyncStatus should be success on initial import")
}
...
func TestCardRepository_FindAllCards(t *testing.T) {
	...
	t.Run("respects pagination with ordering", func(t *testing.T) {
		...
		assert.Equal(t, allIDs[0], page1IDs[0], "First page should contain the first card ResourceID")

		require.Len(t, page2IDs, 1)
		assert.Equal(t, allIDs[1], page2IDs[0], "Second page should contain the second card ResourceID")

		require.Len(t, page3IDs, 1)
		assert.Equal(t, allIDs[2], page3IDs[0], "Third page should contain the third card ResourceID")
	})
