...
func (_q *TagQuery) QueryCards() *CardQuery {
...
// FirstID returns the first Tag ResourceID from the query.
// Returns a *NotFoundError when no Tag ResourceID was found.
func (_q *TagQuery) FirstID(ctx context.Context) (id scheme.TagID, err error) {
	var ids []scheme.TagID
...
func (_q *TagQuery) FirstIDX(ctx context.Context) scheme.TagID {
...
// OnlyID is like Only, but returns the only Tag ResourceID in the query.
// Returns a *NotSingularError when more than one Tag ResourceID is found.
// Returns a *NotFoundError when no entities are found.
func (_q *TagQuery) OnlyID(ctx context.Context) (id scheme.TagID, err error) {
	var ids []scheme.TagID
...
func (_q *TagQuery) OnlyIDX(ctx context.Context) scheme.TagID {
...
func (_q *TagQuery) IDs(ctx context.Context) (ids []scheme.TagID, err error) {
...
func (_q *TagQuery) IDsX(ctx context.Context) []scheme.TagID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *TagQuery) WithCards(opts ...func(*CardQuery)) *TagQuery {
	query := (&CardClient{config: _q.config}).Query()
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withCards; query != nil {
		if err := _q.loadCards(ctx, query, nodes,
			func(n *Tag) { n.Edges.Cards = []*Card{} },
			func(n *Tag, e *Card) { n.Edges.Cards = append(n.Edges.Cards, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadCards(ctx context.Context, query *CardQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Card)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[scheme.TagID]*Tag)
	nids := make(map[scheme.CardID]map[*Tag]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.CardsTable)
		s.Join(joinT).On(s.C(card.FieldID), joinT.C(tag.CardsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.CardsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.CardsPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				outValue := scheme.TagID(values[0].(*sql.NullString).String)
				inValue := *values[1].(*scheme.CardID)
				if nids[inValue] == nil {
