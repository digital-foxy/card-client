...
type Tag struct {
	config `json:"-"`
	// ID of the ent.
	ID scheme.TagID `json:"id,omitempty"`
...
func (e TagEdges) CardsOrErr() ([]*Card, error) {
	if e.loadedTypes[0] {
		return e.Cards, nil
	}
	...
	for i := range columns {
		...
		case tag.FieldID:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.ID = scheme.TagID(value.String)
			}
		case tag.FieldName:
			if value, ok := values[i].(*sql.NullString); !ok {
...
// QueryCards queries the "cards" edge of the Tag entity.
func (_m *Tag) QueryCards() *CardQuery {
	return NewTagClient(_m.config).QueryCards(_m)
}
