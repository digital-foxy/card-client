...
func (_q *CardQuery) QueryTags() *TagQuery {
...
// FirstID returns the first Card ResourceID from the query.
// Returns a *NotFoundError when no Card ResourceID was found.
func (_q *CardQuery) FirstID(ctx context.Context) (id scheme.CardID, err error) {
	var ids []scheme.CardID
...
func (_q *CardQuery) FirstIDX(ctx context.Context) scheme.CardID {
...
// OnlyID is like Only, but returns the only Card ResourceID in the query.
// Returns a *NotSingularError when more than one Card ResourceID is found.
// Returns a *NotFoundError when no entities are found.
func (_q *CardQuery) OnlyID(ctx context.Context) (id scheme.CardID, err error) {
	var ids []scheme.CardID
...
func (_q *CardQuery) OnlyIDX(ctx context.Context) scheme.CardID {
...
func (_q *CardQuery) IDs(ctx context.Context) (ids []scheme.CardID, err error) {
...
func (_q *CardQuery) IDsX(ctx context.Context) []scheme.CardID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *CardQuery) WithTags(opts ...func(*TagQuery)) *CardQuery {
	query := (&TagClient{config: _q.config}).Query()
...
//
//	var v []struct {
//		ImportIndex int `json:"batch_order,omitempty"`
//		Count int `json:"count,omitempty"`
//	}
//
//	client.Card.Query().
//		GroupBy(card.FieldBatchOrder).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := _q.withTags; query != nil {
		if err := _q.loadTags(ctx, query, nodes,
			func(n *Card) { n.Edges.Tags = []*Tag{} },
			func(n *Card, e *Tag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CardQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Card, init func(*Card), assign func(*Card, *Tag)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[scheme.CardID]*Card)
	nids := make(map[scheme.TagID]map[*Card]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(card.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(card.TagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(card.TagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(card.TagsPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.ScanValues = func(columns []string) ([]any, error) {
				...
				return append([]any{new(scheme.CardID)}, values...), nil
			}
			spec.Assign = func(columns []string, values []any) error {
				outValue := *values[0].(*scheme.CardID)
				inValue := scheme.TagID(values[1].(*sql.NullString).String)
				if nids[inValue] == nil {
